...
// 새 객체 생성
exports.DietScheduler = async (req,res)=>{
    if(!req.body){
        res.status(400).send({
    ...
    if (!new_scheduler.data) {
        const scheduler = new Scheduler({
            id: req.body.id || 0, 
            time: scheduler_plus_diet_time ,
        })
        const result = await Scheduler.create(scheduler);
        res.status(200).json({ message: "create scheduler", data: {diet: timetoArray , isSignIn:true, SessionID:1}  });
    } else {
        const result__ = await Scheduler.updateByID( scheduler_plus_diet_time, id)
        res.status(200).json({ message: "Success", data: {diet: timetoArray, isSignIn:true, SessionID:1}  });
    }
}